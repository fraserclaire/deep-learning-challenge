...
An alternative model that can be used to solve this problem is the Random Forest Classifier. This method is useful because it highlights which features are informative, therefore the process described above can be achieved without the need to eliminate each variable one-by-one.
